...
const EditCouponModal = ({ isOpen, onClose, onSuccess, couponData }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        if (couponData) {
            setFormData({
                ...
                specificItems: couponData.specificItems || [],
                itemModel: couponData.itemModel || 'Course'
            });
        }
    }, [couponData]);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await api.patch(`/api/coupons/${couponData._id}`, formData);
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                    ...
                    <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-emerald-50/20">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                                Edit Coupon
                                <span className="text-secondary text-[10px] font-black uppercase bg-white border border-slate-100 px-4 py-1.5 rounded-xl shadow-inner-sm">{formData.code}</span>
                            </h2>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Modify current coupon configuration</p>
                        </div>
                        <button 
                            onClick={onClose}
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-hide">
                        {/* Status Toggle */}
                        <div className="flex items-center justify-between p-8 bg-slate-50/50 rounded-[2.5rem] border border-slate-100 shadow-inner-sm">
                            <div className="space-y-1">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block leading-none italic">Configuration Status</span>
                                <h4 className={`text-sm font-black uppercase tracking-widest mt-1 ${formData.status === 'active' ? 'text-emerald-600' : 'text-amber-600'}`}>{formData.status}</h4>
                            </div>
                            <div className="flex gap-2">
                                {['active', 'paused'].map((status) => (
                                    <button 
                                        ...
                                        className={`px-6 h-12 rounded-xl font-black text-[10px] uppercase transition-all flex items-center gap-2 shadow-sm ${formData.status === status ? (status === 'active' ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-amber-500 text-white shadow-amber-200') : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'}`}
                                    >
                                        <div className={`w-2 h-2 rounded-full ${formData.status === status ? 'bg-white' : (status === 'active' ? 'bg-emerald-400' : 'bg-amber-400')}`} />
                                        {status}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3 opacity-60">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 leading-none italic">Coupon Code (Locked)</label>
                                <input 
                                    disabled
                                    type="text" 
                                    className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 font-bold text-slate-300 uppercase shadow-inner-sm cursor-not-allowed"
                                    value={formData.code}
                                />
                            </div>
                            <div className="space-y-3">
                                ...
                                <textarea 
                                    ...
                                    className="w-full h-24 bg-white border-2 border-slate-100 rounded-2xl p-6 font-bold text-slate-700 focus:outline-none focus:ring-8 focus:ring-primary/5 transition-all shadow-inner-sm hover:border-slate-200 resize-none"
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            className={`flex-1 h-12 rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-all ${formData.discountType === 'percentage' ? 'bg-slate-900 text-white shadow-lg shadow-slate-200' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'}`}
                                        >
                                            <Percent size={14} />
                                            Percentage
                                        </button>
                                        <button 
                                            ...
                                            className={`flex-1 h-12 rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-all ${formData.discountType === 'fixed' ? 'bg-slate-900 text-white shadow-lg shadow-slate-200' : 'bg-white text-slate-400 border border-slate-100 hover:bg-slate-50'}`}
                                        >
                                            <IndianRupee size={14} />
                                            Fixed Amount
                                        </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-4">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1 leading-none italic">
                                    Expiry Date
                                    <Calendar size={12} className="text-slate-300" />
                                </label>
                                <input 
                                    type="date" 
                                    className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 font-bold text-slate-700 focus:outline-none focus:ring-8 focus:ring-primary/5 transition-all shadow-inner-sm"
